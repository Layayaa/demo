    ...
    try:
        ...
        try:
                    ...
                    try:
                        ...
                        for i in range(1, 4):  # 支持最多3个供应商
                            ...
                            price_record = PriceRecord(
                                ...
                                engineer_name=get_cleaned_value(row, '填报工程师') or engineer_name,
            ...
            recent_queries = QueryLog.query.filter(
                QueryLog.engineer_name == engineer_name,
            ...
            inquiry_file.parse_status = f'failed: {str(e)}'
            db.session.commit()
...
def natural_language_query():
    ...
    try:
        ...
        if request.is_json:
            ...
            data = request.get_json(force=True, silent_errors=True)
...
def get_record_detail(record_id):
    ...
    try:
        ...
        source_file = get_source_file_info(record.file_id)
        if source_file:
            result['source_file_info'] = source_file.to_dict()
...
def download_file(file_id):
    ...
    try:
        ...
        for filename in os.listdir(upload_dir):
            if filename.endswith('_' + inquiry_file.file_name):
...
def create_initial_admin():
    ...
    try:
        ...
        if user_count == 0:
            ...
            admin.set_password('138000')  # 手机号后六位
            db.session.add(admin)
            db.session.commit()
            print("[初始化] 已创建默认管理员账号:")
            print("  - 手机号: 13800138000")
            print("  - 密码: 138000 (手机号后六位)")
            print("  - 请登录后及时修改密码")
    except Exception as e:
